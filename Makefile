...
MANPAGES ?= $(patsubst %.asciidoc.in,%,$(wildcard ./docs/man/man1/sshgo*.1.asciidoc.in))
...
# sshgo-test parameters
SSHGO_TEST ?= test/runner/sshgo-test
TEST_FLAGS ?=

# sshgo-test units parameters (make test / make test-py3)
...
# sshgo-test integration parameters (make integration)
IMAGE ?= centos7
TARGET ?=
...
tests:
	$(sshgo_TEST) units -v --python $(PYTHON_VERSION) $(TEST_FLAGS)

tests-py3:
	$(sshgo_TEST) units -v --python $(PYTHON3_VERSION) $(TEST_FLAGS)

integration:
	$(sshgo_TEST) integration -v --docker $(IMAGE) $(TARGET) $(TEST_FLAGS)
...
pep8:
	$(sshgo_TEST) sanity --test pep8 --python $(PYTHON_VERSION) $(TEST_FLAGS)

pyflakes:
	pyflakes lib/sshgo/*.py lib/sshgo/*/*.py bin/*

clean:
	@echo "Cleaning up distutils stuff"
	rm -rf build
	rm -rf dist
	rm -rf lib/sshgo.egg-info/
	@echo "Cleaning up byte compiled python stuff"
	find . -type f -regex ".*\.py[co]$$" -delete
	find . -type f -regex ".*\.[co]$$" -delete
...
generate_asciidoc: lib/sshgo/cli/*.py
